...
var jaservAjax = function (objectData){
    ...
    this.postFalse = function(){
        ...
        tempAjax.onreadystatechange = function(){
            ...
            }else if(tempAjax.readyState == 4 && tempAjax.status > 400 && tempAjax.status != 404 && tempAjax.status != 403){
                tempThis.postFalse();
            }else if(tempAjax.readyState == 4 && (tempAjax.status != 404 || tempAjax.status != 403)){
                tempThis.funcFalse('Page Not found error kode '+tempAjax.status);
            }else{
    ...
    this.postTrue = function(){
        ...
        tempAjax.onreadystatechange = function(){
			//alert(tempAjax.readyState+" "+tempAjax.status);
            if((tempAjax.readyState == 4)&&(tempAjax.status == 200)){
				
                tempResponseText = tempAjax.responseText;
				//alert(tempThis.isInFalse(tempResponseText[0]));
                tempThis.funcProcess(tempResponseText.substr(1,tempResponseText.length-1));
                if(tempThis.isInFalse(tempResponseText[0])){
                    tempThis.funcFalse(tempResponseText.substr(1,tempResponseText.length-1));
                }else if(tempThis.isInTrue(tempResponseText[0])){
    ...
    if(objectData){
        ...
        if(typeof objectData.codeReloadFuncTrue  !== 'undefined'){ this.codeReloadFuncTrue = objectData.codeReloadFuncTrue;}else{this.codeReloadFuncTrue = settingJaservAjax.codeReloadFuncTrue;}
        if(typeof objectData.codeReloadFuncFalse !== 'undefined'){ this.codeReloadFuncFalse = objectData.codeReloadFuncFalse;}else{this.codeReloadFuncFalse = settingJaservAjax.codeReloadFuncFalse;}
        if(typeof objectData.reloadFunc !== 'undefined'){ this.reloadFunc = objectData.reloadFunc;}else{this.reloadFunc = settingJaservAjax.reloadFunc;}
