	...
	this.add = function(){
		ModalControl.openListModal("Menambahkan",{
			1 : {
				...
				ico : "md md-person",
				act : function(){
					ModalControl.openFormModal("Sekolah",
					function(){
						
					},function(){
						$(
							"#idline,#nama,#nip,#nohp,#email,#formModalButtonNo,#formModalButtonYes,#kepsek"
						).attr("disabled","true");
			...
			2:{
				...
				ico : "md md-people",
				act : function(){
					ModalControl.openFormModal("Guru",
					function(){
						$('#sekolah').select2({
			...
			3:{
				...
				act : function(){
					...
					function(){
						AkunAddVariabelGlobal['tempGuru']=[];
						AkunAddVariabelGlobal.tempGuru['defaults']=[];
						AkunAddVariabelGlobal.tempGuru.defaults['listGuru']=$('#listGuru').html();
						AkunAddVariabelGlobal.tempGuru.defaults['listNip']=$('#listNip').html();
						$('#nama').select2({
							dropdownParent : $('#formModal')
						});
						$('#guru').select2({
						...
						$("#nama").on("change",function(){
							var nama = $(this).val();
							if(nama == ""){
								$('#listGuru').html(AkunAddVariabelGlobal.tempGuru.defaults['listGuru']);								
								$('#guru').select2({
									dropdownParent : $('formModal')
								});
								if($('#guru').val() == ""){
									$(
										"#idline,#nip,#nohp,#email, #formModalButtonYes"
									).attr("disabled","true");
								}
						...
						var e = new jaservAjax({
							...
							funcTrue : function(message){
								ToasterControl.openSuccessMessage({	message : "berhasil ...",clear : true,timeOut : 2000});
								$(
									"#idline,#wali,#nama,#nip,#nohp,#email,#formModalButtonNo,#formModalButtonYes,#kepsek"
								).removeAttr("disabled");
								AkunControl.getCoul(false);
								ModalControl.closeFormModal();
							},
