...
EvenControlClass = function(){
	this.refreshContent = function(){
		if(EvenVariabelGlobal['idActive'] == "*"){
			//alert($("#main-event").val());
			EvenVariabelGlobal['idActive'] = $("#main-event").val();
		}
		if(EvenVariabelGlobal['idSekolah'] == "*"){
		...
		var e = new jaservAjax({
			...
			reloadFunc : false,
			content : "kodeForm=J453RVT3CH@W3N4@FORM&keyword="+EvenVariabelGlobal['keyWord']+"&guru="+EvenVariabelGlobal['idActive']+"&sekolah="+EvenVariabelGlobal['idSekolah'],
	...
	this.refreshSubEvent = function(){
		var e = new jaservAjax({
			...
			content : "kodeForm=J453RVT3CH@W3N4@FORM&"+
				"nama="+EvenVariabelGlobal['idSekolah']
				,
			funcTrue : function(message){
				
				$('#teach-id-list-member').html(message);
				$('#teach-id-list-member').select2({
